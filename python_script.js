...
let userId = localStorage.getItem('pythonRoadmapUserId');
if (!userId) {
    userId = 'user_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9);
    localStorage.setItem('pythonRoadmapUserId', userId);
}

const USER_DOC_REF = doc(db, "roadmap_users", "python_" + userId);
...
function updateStats() {
    ...
    // Assume up to 15 phases for python
    for (let phaseId = 0; phaseId <= 15; phaseId++) {
...
async function restoreProgress() {
    try {
        ...
        if (docSnap.exists()) {
            ...
            const localProgress = JSON.parse(localStorage.getItem('pythonRoadmapProgress'));
            if (localProgress) {
                progress = localProgress;
                startDate = localStorage.getItem('pythonRoadmapStartDate');
...
async function fetchRoadmapData() {
    try {
        const docRef = doc(db, "roadmap_content", "python_main");
